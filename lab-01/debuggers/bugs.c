...
struct Storage {
  ...
  /* FIX
    Increased size of array to 7 to allow last index to be NULL
  */
...
void echoohce(char** strs) {
  ...
  */
  ...
  /* FIXES
    Changed *iter to iter (because we need to compare pointer addresses)
    Changed NULL to stop_beginning (marks the memory address prior to the strs pointer in memory)
  */
...
int main(int argc, char** argv) {
  ...
  /* FIX
    Must initialize the value of bug_info.num_bugs_on_mars to 0
  */
  ...
  /* FIX
    Must include this due to echo functions accepting only NULL-terminated strings 
  */
  ...
  /* Must free this before re-assigning the pointer*/
  ...
  /* Don't need to free the NULL defined pointer (index 6 of sentence array) */

  /* FIX
    Assign each sentence pointer to NULL, otherwise there are dangling pointers
  */
  for(int i = 0; i < 6; ++i) {
  ...
  /* FIX - remove the following line (it is not a heap-allocated pointer)...
