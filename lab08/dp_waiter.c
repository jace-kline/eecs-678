...
// Marks the corresponding chopsticks_available with the given status (0 = TAKEN) (1 = AVAILABLE)
// based on the philosopher's position at the table
void mark_chops(int status, philosopher* p) {
  available_chopsticks[p->id] = status;
  available_chopsticks[p->id == 0 ? NUM_CHOPS-1 : (p->id) - 1] = status;
}

int both_chops_available (philosopher *p) {
...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    mark_chops(AVAILABLE, me);
    // pthread_mutex_unlock(right_chop(me));
    // pthread_mutex_unlock(left_chop(me));

    // signal left and right philosophers to try to eat
    // Recall that they will be checking whether they can actually eat
    // regardless of if they receive a signal
