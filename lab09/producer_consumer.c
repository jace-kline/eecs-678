...
void *producer (void *parg)
{
  ...
  while (1) {
     ...
     * produce, so wait until it is not full. We must avoid
	 * the busy waiting being done here. Also note that, if we
...
void *consumer (void *carg)
{
  ...
  while (1) {
     ...
     * produce, so wait until it is not empty. We must avoid
	 * the busy waiting being done here. Also note that, if we
	 * get stuck in here, somebody should wake us.
     */
...
int main (int argc, char *argv[])
{
  ...
  // My solution: Initialize the semaphores of the fifo queue struct

  // No slots to be read => initial value = 0
  sem_init(fifo->slotsToGet, 0, 0);
  // QUEUESIZE slots to be filled => initial value = QUEUESIZE
  sem_init(fifo->slotsToPut, 0, QUEUESIZE);
